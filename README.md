...
| QR Reference profile | standard QR v27-L writer + local ZXing reader; same RaptorQ, manifest, and BLAKE3 stream |
...
For an optical A/B baseline, choose **QR Reference** on the skin and **QR Reference ·
standard QR** in the eye's receiver mode before starting the camera. It uses a fixed
version-27, ECC-L, mask-4 QR symbol: 125 × 125 modules plus its white four-module quiet
zone. The QR layer alone changes; the file still goes through Cuttldrop's compression,
RaptorQ, manifest, CRC gate, and mandatory BLAKE3 verification. The ZXing reader WASM
ships inside the web build, so it does not fetch a decoder from a CDN while receiving.
...
There is also a deliberately conventional **QR Reference** profile. It wraps those same
framed fountain packets in a fixed standard QR v27-L symbol, generated by `qrcode` and
decoded by the locally bundled `zxing-wasm` reader. Its white quiet zone, standard
finder/alignment patterns, and mature detector make it the control experiment for the
custom chroma-cell renderer—not a replacement for it. If the reference profile works in
the same physical setup and a custom profile does not, the evidence points at Cuttldrop's
optical raster rather than its transport or file-integrity layers.
